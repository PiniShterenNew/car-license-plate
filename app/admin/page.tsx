...
export default async function AdminDashboardPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-4 sm:gap-6 md:gap-8 w-full">
      {/* Page Heading & Controls */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h3 className="text-2xl sm:text-3xl font-black text-[#121417] dark:text-white">ביצועי מערכת</h3>
          <p className="text-sm sm:text-base text-[#657586] dark:text-slate-400">נתונים בזמן אמת על שאילתות ואימותי רישוי</p>
        </div>
        <div className="flex items-center bg-white dark:bg-slate-900 p-1 rounded-lg border border-[#e5e7eb] dark:border-slate-800 shadow-sm w-full md:w-auto transition-colors">
          <Link
            href="?range=7d"
            className={`flex-1 md:flex-none px-3 sm:px-4 py-2 rounded text-xs sm:text-sm font-bold transition-colors text-center ${
              range === '7d'
                ? 'bg-primary text-white shadow-sm'
                : 'text-[#657586] dark:text-slate-400 hover:bg-[#f0f2f4] dark:hover:bg-slate-800 font-medium'
            }`}
          >
            7 ימים
          </Link>
